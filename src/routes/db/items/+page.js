...
function getItemCategory(category) {
    // Category 0 = 消費アイテム Consumables
    // Category 1 = 売却専用アイテム Sell items (fish, luno gems)
    // Category 2 = アビリティプラグ Plugs
    // Category 3 = 汎用素材アイテム General purpose item (materials, etc)
    // Category 4 = イデア Idea
    // Category 5 = セットボックス Set box
    // Category 6 = セレクトボックス Selection Box
    // Category 7 = ランダムボックス Random Box (random plug..)

    switch (category) {
        case 0: return `Consumable`
        case 1: return `Sell-only item`
        case 2: return `Weapon Plug`
        case 3: return `General Item`
        case 4: return `Idea`
        case 5: return `Set Box`
        case 6: return `Selection Box`
        case 7: return `Random Box`
    }
}
