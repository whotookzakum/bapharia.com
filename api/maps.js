...
// Use key instead of Id because some keys don't match their Ids, like "pub0801": { "Id": "pat0801_Top" }
const MAPS = Object.entries(DT_ZoneDB[0].Rows).map(([id, data]) => ({ id, name: data.NameTextId }));

const twr003_EN = Object.values(import.meta.glob("./bp_client/bno/Content/Maps/twr/twr003/Sublevel/*.json", { import: "default", eager: true }))[0]
    .filter(obj => obj.Properties?.RelativeLocation)
    .map(obj => {
        return {
            ...obj,
            coords: [
                obj.Properties.RelativeLocation.X,
                obj.Properties.RelativeLocation.Y,
                obj.Properties.RelativeLocation.Z,
            ],
            id: obj.Outer,
            category: "hi",
            iconName: "キャンプ",
...
function processMap(map, lang) {
    console.log(uniq(twr003_EN.map(obj => obj.Type)))
    return {
        ...map,
        markers: twr003_EN
    }
...
export function getEntry(id, lang) {
    return processMap(MAPS.find(map => map.id === id), lang)
}

const maps = (lang) => MAPS.map(map => processMap(map, lang))
...
const allMapFiles = import.meta.glob("./bp_client/bno/Content/Maps/**/**/(s|S)ublevel/*.json")
